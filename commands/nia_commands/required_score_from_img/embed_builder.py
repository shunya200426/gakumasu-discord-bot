...
def build_error_embed(*, params: dict, bonus: dict | None = None, is_boost: bool = False) -> Embed:
    """
    OCR読み取りエラー時に、取得できた/できなかった値を1枚のEmbedで俯瞰表示する。
    - params: {"vo", "da", "vi", "fans"}（Noneあり）
    - bonus:  {"vo","da","vi", ...}（任意, None可）
    - scores: {"sum_score","vo","da","vi"}（任意, None可）
    """
    embed = Embed(
